...
public class MinMax implements AIAlgorithm {
	...
	public MinMax(int depth, boolean useAblphaBeta) {
		if (depth < 1) {
			throw new GameError("Invalid depth ('" + depth
					+ "') for the MinMax algorithm, it should be at east 1");
		}
		this.depth = depth;
	...
	@Override
	public GameMove getMove(Piece p, Board board, List<Piece> pieces,
			GameRules rules) {
		try {
			Pair<GameMove, Double> m = minmax(depth, -Double.MAX_VALUE,
					Double.MAX_VALUE, p, p, board, pieces, rules);
			return m.getFirst();
		} catch (Exception e) {
			return null;
		}
	...
	private Pair<GameMove, Double> minmax(int d, Double alpha, Double beta,
			Piece p, Piece turn, Board board, List<Piece> pieces,
			GameRules rules) throws InterruptedException {
		...
		if (d < 1) {
			return new Pair<GameMove, Double>(null, rules.evaluate(board,
					pieces, turn, p));
		}
		...
		assert (moves.size() > 0);
		if (moves.size() == 0) {
			System.err.println("NO MOVESSSSSSSSSSSSSSSSSSSS");
		}
	...
	private Pair<GameMove, Double> max(int d, Double alpha, Double beta,
			Piece p, Piece turn, Board board, List<Piece> pieces,
			GameRules rules, List<GameMove> moves) throws InterruptedException {
		...
		for (GameMove m : moves) {
			...
			Pair<GameMove, Double> res = minmax(d - 1, alpha, beta, p,
					nextTurn, b, pieces, rules);
	...
	private Pair<GameMove, Double> min(int d, Double alpha, Double beta,
			Piece p, Piece turn, Board board, List<Piece> pieces,
			GameRules rules, List<GameMove> moves) throws InterruptedException {
