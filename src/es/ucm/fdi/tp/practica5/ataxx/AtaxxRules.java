...
public class AtaxxRules implements GameRules {
	...
	public Piece nextPlayer(Board board, List<Piece> playersPieces,
		...
		for (int j = 0; j < pieces.size(); j++) {
			if (!validMoves(board, playersPieces, next).isEmpty()) {
				return next;
			} else {
				i++;
	...
	public double evaluate(Board board, List<Piece> pieces, Piece turn, Piece p) {
		...
		int misPiezas = board.getPieceCount(p);
		int misPiezasSeguras = piezasSeguras(board, p);
		int susPiezas = 0;
		int susPiezasSeguras = 0;
		int totales;

		for (Piece q : pieces) {
			if (!p.equals(q)) {
				int x = board.getPieceCount(q);
				int y = piezasSeguras(board, q);
				if (x > susPiezas) {
					susPiezas = x;
				}
				if (y > susPiezasSeguras) {
					susPiezasSeguras = y;
				}
			}
		}

		totales = misPiezas + susPiezas;
		return (misPiezas - susPiezas) / totales * 0.8
				+ (misPiezasSeguras / misPiezas - susPiezasSeguras / susPiezas)
				* 0.2;
	}

	private int piezasSeguras(Board b, Piece p) {
		int res = b.getPieceCount(p);
		for (int row = 0; row < dim; row++) {
			for (int col = 0; col < dim; col++) {
				if (b.getPosition(row, col) != null
						&& b.getPosition(row, col).equals(p)) {
					for (int i = -3; i < 4; i++) {
						for (int j = -3; j < 4; j++) {
							if (row + i < dim
									&& row + i > -1
									&& col + j < dim
									&& col + j > -1
									&& (b.getPosition(row + i, col + j) != null && !b
											.getPosition(row + i, col + j)
											.equals(p))) {
								res--;
								i = 4;
								j = 4;
